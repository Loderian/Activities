...
function acts_activity_nice_management( $activity, $current_url = null ) {
    ...
    if ( $current_url != null ) {
        add_thickbox();
        $output .= activities_user_quick_edit( $nice_settings );
        $output .= acts_report_plan_session_edit_box();
    }
    ...
    $output .= '<li><i class="acts-grey">' . esc_html__( 'Separate multiple fields in one line by a comma.', 'activities' ) . '</i></li>';
    $output .= '</ul>';
 ...
 * @param bool $sort Sort members
 *
 * @return  array   A list of user info to display in column 1 ('col1') and column 2 ('col2')
 */
 ...
 *
 * @param string $selected Which prepared setup is selected
 *
 ...
 * @param array $custom_fields Custom fields to get
 *
 * @return array|false Array of user data, false on user not found
 */
